...
| `SELLPRO_SMTP_HOST` / `SELLPRO_SMTP_USER` / `SELLPRO_SMTP_PASS` | お問い合わせメール送信のSMTP（この3つで有効化） | 自社サーバー(`server.js`)・Vercel(`api/contact.js`)の両方で使用。届け先は `src/data/site.json` の `contact.email`。未設定なら mailto フォールバック |
| `SELLPRO_SMTP_PORT` / `SELLPRO_SMTP_SECURE` / `SELLPRO_SMTP_FROM` | SMTP ポート / TLS(`false`でSTARTTLS) / 差出人 | 既定 `465` / `true` / USERと同じ。詳細は `README.md`「お問い合わせメール（SMTP）」 |
...
- **問い合わせ先・送信設定**：届け先は `src/data/site.json` の `contact.email`（管理画面で編集可）。正しい運用窓口かを公開前に確認してください。フォームは `/api/contact` 経由で **SMTP メール送信する本実装**です。公開前に上記 SMTP 環境変数（`SELLPRO_SMTP_*`）を設定し、テスト送信で届くか確認してください（未設定時は mailto フォールバックで動作）。
