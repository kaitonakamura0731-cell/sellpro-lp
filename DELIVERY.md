...
| 変数 | 用途 | 備考 |
|---|---|---|
| `SELLPRO_ADMIN_PASSWORD` | 管理画面のログインパスワード | 任意の十分に長い文字列 |
| `ADMIN_PATH` | 管理画面の秘匿パス（推奨） | 例 `/x7k2-manage/`。設定すると素の `/admin/` が 404 になり外部から存在を気づかれにくくなる。未設定時は `/admin/` で配信（開発用）。先頭・末尾スラッシュの有無は自動正規化。**Vercel環境では不使用**（`server.js` 専用） |
| `GITHUB_CONTENT_TOKEN` | CMSが `src/data/*.json` と `public/` をコミットするためのGitHub PAT | **本リポジトリへ write 権限**のあるトークン（Fine-grained PAT 推奨）。**Vercel環境のみ** |
| `GITHUB_OWNER` | リポジトリの owner | 省略時の既定はコード内に記載。**移管後は自分の値に上書き必須**。**Vercel環境のみ** |
| `GITHUB_REPO` | リポジトリ名 | 同上 |
| `GITHUB_BRANCH` | 対象ブランチ | 省略時 `main` |
| `SELLPRO_SMTP_HOST` / `SELLPRO_SMTP_USER` / `SELLPRO_SMTP_PASS` | お問い合わせメール送信のSMTP（この3つで有効化） | 自社サーバー(`server.js`)・Vercel(`api/contact.js`)の両方で使用。未設定なら mailto フォールバック |
| `SELLPRO_SMTP_PORT` / `SELLPRO_SMTP_SECURE` / `SELLPRO_SMTP_FROM` | SMTP ポート / TLS / 差出人 | 既定 `465`。`SELLPRO_SMTP_SECURE` 未指定時はポート465ならTLS有効・それ以外は STARTTLS（`false` 相当）。`FROM` 既定は USER。詳細は `README.md`「お問い合わせメール（SMTP）」 |
| `SELLPRO_CONTACT_EMAIL` | 任意（Vercel運用時の届け先） | 問い合わせの届け先メール。設定時は `src/data/site.json` の `contact.email` より**優先**。自社サーバー(`server.js`)方式では `site.json` をそのまま読むため不要 |
...
- **(A) 自社サーバーでCMSも使う（同梱の `server.js` を使えば配線不要）**：本納品物には自己完結サーバー `server.js` を同梱しています。GitHub / Vercel は不要で、次の3手順だけで `http://localhost:4321` にLPと管理画面が両方立ち上がります。
  1. `npm install`（依存取得。`server.js` 自体は追加依存なし＝Node組み込みのみ）
  2. `npm run build`（初回のみ。`dist/` が無ければ `server.js` 起動時に自動実行されます）
  3. `ADMIN_PATH=/秘匿パス/ SELLPRO_ADMIN_PASSWORD=xxx npm run serve`（`ADMIN_PATH` は管理画面の秘匿URL パス。例: `/x7k2-manage/`。`xxx` はログインパスワード。ポート変更は先頭に `PORT=8080` を追加）
     - **お問い合わせメールも送る場合**は、同じ起動コマンドに SMTP 環境変数を足す:
       ```
       ADMIN_PATH=/秘匿パス/ SELLPRO_ADMIN_PASSWORD=xxx \
         SELLPRO_SMTP_HOST=smtp.example.com SELLPRO_SMTP_USER=送信メール SELLPRO_SMTP_PASS=パスワード \
         npm run serve
       ```
       届け先は `src/data/site.json` の `contact.email`。未設定なら mailto フォールバック。Gmail/Workspace は『アプリパスワード』が必要（`README.md`「お問い合わせメール（SMTP）」参照）。

  管理画面で保存すると、`server.js` が**ローカルの `src/data/*.json`（画像/PDFは `public/<folder>/`）を直接更新し、自動で `npm run build` を実行して `dist/` に反映**します。GitHub へのコミットも外部CIも不要です。常時運用する場合は `npm run serve` を systemd / pm2 等のプロセス管理下に置き、リバースプロキシ（nginx 等）で公開してください。さらに堅くするなら nginx 等で管理画面パス（`ADMIN_PATH`）と API（`/api/admin-*`）に IP制限または Basic 認証をかけることを推奨します。
...
- **問い合わせ先・送信設定**：届け先は `src/data/site.json` の `contact.email`（**ファイルを直接編集**。管理画面のタブはありません）。既定はテスト用アドレスのため、正しい運用窓口へ公開前に差し替えてください。フォームは `/api/contact` 経由で **SMTP メール送信する本実装**です。公開前に上記 SMTP 環境変数（`SELLPRO_SMTP_*`）を設定し、テスト送信で届くか確認してください（未設定時は mailto フォールバックで動作）。
